    ...
    try {
      ...
      const validPlaces =
        data.elements?.filter(
          (place: any): place is MedicalPlace =>
            place?.id &&
            place.type &&
            place.tags &&
      ...
      console.error("[fetchMedicalPlaces] Failed to load locations:", err);
      if (err.name === "AbortError" || err.name === "TimeoutError") {
        setError("Fetching locations timed out. Please try again.");
      } else {
        setError(
          err.message || "An unknown error occurred while fetching locations."
        );
      }
      setPlaces([]);
    } finally {
